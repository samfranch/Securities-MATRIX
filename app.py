...
def find_search_result(results: List[SearchResult], label: Optional[str]) -> Optional[SearchResult]:
    if not label:
        return None
...
def render_search_panel() -> None:
    ...
    selected_label = st.selectbox(
        ...
        index=None,
        placeholder="Selectionnez un resultat...",
        key="selected_search_label",
    )

    selected_result = find_search_result(results, selected_label)
    if selected_result is not None:
        st.markdown(
            f"**{selected_result.display_name}**  \n"
            f"`{selected_result.symbol}` · {selected_result.exchange} · {selected_result.quote_type} · {selected_result.region}"
        )
        if st.button(f"Ajouter {selected_result.symbol}", type="primary"):
            add_symbol(selected_result.symbol)
            st.rerun()
